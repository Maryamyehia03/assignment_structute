...
bool sort_name(student& s1,student& s2){cnt2++;
    return s1.getname()<s2.getname();
}

bool sort_gpa(student& s1, student& s2){cnt1++;
...
void insertionSort(vector<student>& students,u compare) {
    student tmp;
    for (int i = 0; i < students.size(); ++i) {cnt1++;cnt2++;
        tmp = students[i];
        int j = i - 1;
        while (j >= 0 && compare(tmp,students[j])) {cnt1++;cnt2++;
...
void countsort_gpa(vector<student>vec) {
    ...
    // Count items
    for (int i = 0; i < vec.size(); i++) {cnt1++,cnt2++;
        counts[int (vec[i].getgpa())]+=1;
    }

    // Compute cumulative counts
    for (int i = 1; i <= max; i++) {cnt1++,cnt2++;
        counts[i] += counts[i - 1];
    }
    ...
    // Order elements
    for (int i = vec.size()-1; i >= 0; i--) {cnt1++,cnt2++;
...
void Merge(vector<student>&array,int left,int middle,int right,u compare)
{
    ...
    vector<student>l(n1);
    vector<student>m(n2);
    for(int i = 0;i<n1;i++)l[i] = array[i+left],cnt1++,cnt2++;
    for(int i = 0;i<n2;i++)m[i] = array[i+middle+1],cnt1++,cnt2++;;
    int i = 0,j = 0,k = left;
    while(i < n1 && j < n2)
    {cnt1++,cnt2++;cnt1++,cnt2++;
        if(compare(l[i],m[j]))
...
void SelectionSort (vector<student>&arr , int n, u compare)
{
    ...
    for (int i = 0 ; i < n ; i++,cnt1++,cnt2++)
    {
        min = i;
        for (int j = i + 1; j < n; j++,cnt1++,cnt2++) // to find min number in every iteration
        {
...
void shellSort(vector<student>& arr, int n, U compare) {
    for (int gap = n / 2; gap > 0; gap /= 2,cnt1++,cnt2++) {
        for (int i = gap; i < n; i++,cnt1++,cnt2++) {
            student temp = arr[i];
            int j;
            for (j = i; j >= gap && compare(temp, arr[j - gap]); j -= gap,cnt1++,cnt2++) {
                arr[j] = arr[j - gap];
            }
            arr[j] = temp;
        }
